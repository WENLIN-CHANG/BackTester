...
const backtestSchema = z
  .object({
    stocks: z
      .array(z.string().min(1, "股票代碼不能為空"))
      .min(1)
      .max(LIMITS.MAX_STOCKS),
    ...
    investment: z.object({
      amount: z
        .number()
        .min(
          LIMITS.MIN_INVESTMENT,
          `投資金額最少為 ${LIMITS.MIN_INVESTMENT.toLocaleString()} 元`,
        )
        .max(
          LIMITS.MAX_INVESTMENT,
          `投資金額最多為 ${LIMITS.MAX_INVESTMENT.toLocaleString()} 元`,
        )
        .multipleOf(
          LIMITS.MIN_INVESTMENT_STEP,
          `投資金額必須是 ${LIMITS.MIN_INVESTMENT_STEP} 的倍數`,
        ),
    }),
  })
